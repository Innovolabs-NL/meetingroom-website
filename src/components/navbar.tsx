...
export function Navbar() {
  ...
  return (
    <motion.nav
      ...
      className="fixed top-0 left-0 right-0 z-50 border-b border-border/50 bg-background/75 backdrop-blur-xl supports-[backdrop-filter]:bg-background/60"
    >
        ...
        <div className="hidden items-center gap-1 md:flex">
          {links.map((link) => (
            <MotionLink
              key={link.href}
              href={link.href}
              className="group relative px-3 py-2 text-sm text-muted transition-colors hover:text-foreground"
              whileHover={{ y: -1 }}
            >
              {link.label}
